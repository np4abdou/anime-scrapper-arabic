...
def process_download_list(file_path, output_path, threads_num):
    """
    Process a list of MediaFire links from a file.
    
    Args:
        file_path (str): Path to the file containing MediaFire links
        output_path (str): Output directory for downloads
    ...
    try:
        ...
        print(f"{bcolors.HEADER}Found {total_links} links in {file_path}{bcolors.ENDC}")
        
        for i, link in enumerate(links, 1):
            print(f"{bcolors.HEADER}Processing link {i}/{total_links}: {link}{bcolors.ENDC}")
            
            folder_or_file = findall(
                r"mediafire\.com/(folder|file|file_premium)\/([a-zA-Z0-9]+)", link
            )
            
            if not folder_or_file:
                print(f"{bcolors.FAIL}Invalid link: {link}{bcolors.ENDC}")
                continue
            ...
            if t in {"file", "file_premium"}:
                ...
                print(f"{bcolors.FAIL}Invalid link type: {t}{bcolors.ENDC}")
                
            print(f"{bcolors.OKGREEN}Completed {i}/{total_links} links{bcolors.ENDC}")
...
def main():
    ...
    Usage:
        ...
        python mediafire.py --list <file_path> [-o <output_path>] [-t <num_threads>]

    Arguments:
        mediafire_url (str): The URL of the file or folder to be downloaded from Mediafire.
        --list (str): Path to a file containing MediaFire links to download.
    ...
    Example:
        ...
        To download from a list:
        $ python mediafire.py --list download_this.txt -o /path/to/output -t 20
    """
    parser = ArgumentParser(
        "mediafire_bulk_downloader", usage="python mediafire.py <mediafire_url> or --list <file_path>"
    )
    
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument(
        "mediafire_url", help="The URL of the file or folder to be downloaded", nargs='?', default=None
    )
    group.add_argument(
        "--list", help="Path to a file containing MediaFire links to download", default=None
    )
    ...
    # Process download list if provided
    if args.list:
        process_download_list(args.list, args.output, args.threads)
        print(f"{bcolors.OKGREEN}{bcolors.BOLD}All downloads from list completed{bcolors.ENDC}")
        exit(0)
    
    # Otherwise process single URL
    folder_or_file = findall(
...
def download_file(
        ...
        while True:
            ...
            if current_time - last_update_time > update_interval:
                ...
                sys.stdout.write(f"\r{bcolors.OKBLUE}[{bar}] {percent:.1f}% | "
                                f"{format_size(downloaded)}/{file_size_formatted} | "
                                f"Speed: {format_size(speed)}/s | ETA: {eta}{bcolors.ENDC}")
                sys.stdout.flush()
    ...
    print(f"\n{bcolors.OKGREEN}{filename}{bcolors.ENDC} downloaded | "
